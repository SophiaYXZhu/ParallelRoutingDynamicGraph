...
int main(int argc, char** argv) {
    ...
    cout << "Routing test on large, fully-connected component with a single vertex attached to the core\n";
    ...
    // Core clique on the first N-1 vertices
    for (int u = 1; u < N; u++) {
        for (int v = u + 1; v < N; v++) {
            g.add_edge(u, v);
            edge_count++;
    ...
    // Single bridge edge from leaf 0 into the core (vertex 1)
    g.add_edge(0, 1);
    edge_count++;
    ...
    // All packages start at vertex 0, destinations are random in the core
    std::vector<std::pair<int,int>> pairs;
    pairs.reserve(cfg.num_pairs);

    std::uniform_int_distribution<int> dist_core(1, N - 1);
    for (int i = 0; i < cfg.num_pairs; i++) {
        int s = 0;                    // all from leaf
        int d = dist_core(rng);       // dest in dense core
    ...
    int delivered   = 0;
    int makespan_ts = 0;   // maximum arrival_time among delivered packages
